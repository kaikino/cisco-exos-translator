...
オプションはありません。変換内容の調整はすべてマッピングファイル (5 章) で行います。
...
2. **マッピングファイルの確認・編集** (5 章)。特にアップリンクポートの番号 (`uplinks.start`) は
...
4. **WARNINGS の確認** (6 章)。「Not translated」に挙がった設定は手動で移植が必要です。
...
| `mirror_egress_mode` | SPAN セッション番号 → `acl` または `whole-port` | フィルタ付き SPAN のみ対象。`whole-port` にすると egress 側はポート全体をミラーします (7.6 節) |
